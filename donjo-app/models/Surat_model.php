<?php class Surat_model extends CI_Model
{
	...
	{
		$sql = "SELECT * FROM tweb_surat_format WHERE kunci = 0 AND desa_id = " . $this->config->item('desa_id') . "";
	...
	{
		$data = $this->db->where('kunci', 0)->where('desa_id', $this->config->item('desa_id'))->get('tweb_surat_format')->result_array();
		for ($i = 0; $i < count($data); $i++) {
	...
	{
		$data = $this->db->where('kunci', 0)->where('mandiri', 1)->where('desa_id', $this->config->item('desa_id'))->get('tweb_surat_format')->result_array();
		for ($i = 0; $i < count($data); $i++) {
			$data[$i]['nama_lampiran'] = $this->nama_lampiran($data[$i]['lampiran']);
		}
		return $data;
	...
	{
		$data = $this->db->where('kunci', 0)->where('favorit', 1)->where('desa_id', $this->config->item('desa_id'))->get('tweb_surat_format')->result_array();
		for ($i = 0; $i < count($data); $i++) {
			$data[$i]['nama_lampiran'] = $this->nama_lampiran($data[$i]['lampiran']);
		}
		return $data;
	...
	public function list_anak($id)
	{
		...
		$this->db->where("
			...
			AND kk_level = 4 AND desa_id = " . $this->config->item('desa_id') . "");
		return $this->list_penduduk();
	}
	...
	function pengikut()
	{
		...
		if (count($id_cb)) {
			...
			LEFT JOIN tweb_keluarga k on u.id_kk = k.id
			LEFT JOIN tweb_penduduk_warganegara f on u.warganegara_id = f.id
			WHERE u.nik IN($outp) AND u.desa_id = " . $this->config->item('desa_id') . "";
			$query = $this->db->query($sql);
			$data  = $query->result_array();
		}
		return $data;
	...
	public function list_pamong()
	{
		$sql = "SELECT u.*, p.nama as nama
			...
			WHERE pamong_status = 1 AND u.desa_id = " . $this->config->item('desa_id') . "";
		$query = $this->db->query($sql);
		$data  = $query->result_array();
		for ($i = 0; $i < count($data); $i++) {
	...
	{
		$sql = "SELECT u.* FROM tweb_desa_pamong u WHERE pamong_id = ? AND u.desa_id = " . $this->config->item('desa_id') . "";
		$query = $this->db->query($sql, $id);
		$data = $query->row_array();
		return $data;
	}
	...
	public function get_data_kk($id = 0)
	{
		$sql = "SELECT b.nik_kepala, b.no_kk,b.id AS id_kk, c.nama as kepala_kk, d.*
			...
			WHERE a.id = ? AND a.desa_id = " . $this->config->item('desa_id') . "";
		$query = $this->db->query($sql, $id);
		$data = $query->row_array();
		return $data;
	}
	...
	{
		$sql = "SELECT u.* FROM tweb_penduduk u WHERE id = ? AND u.desa_id = " . $this->config->item('desa_id') . "";
		$query = $this->db->query($sql, $id);
		$data = $query->row_array();
		return $data;
	}
	...
	public function get_data_istri($id = 0)
	{
		$sql = "SELECT u.id
			FROM tweb_penduduk u
			WHERE u.id = (SELECT id FROM tweb_penduduk WHERE id_kk = (SELECT id_kk FROM tweb_penduduk WHERE id = $id AND kk_level = 1 AND desa_id = " . $this->config->item('desa_id') . ") AND kk_level = 3 limit 1 AND desa_id = " . $this->config->item('desa_id') . ")";
		$query = $this->db->query($sql);
		$data  = $query->row_array();
	...
	public function get_data_suami($id = 0)
	{
		$sql = "SELECT u.id
			FROM tweb_penduduk u
			WHERE u.id = (SELECT id FROM tweb_penduduk WHERE id_kk = (SELECT id_kk FROM tweb_penduduk WHERE id = $id AND kk_level = 3 AND desa_id = " . $this->config->item('desa_id') . ") AND kk_level = 1 limit 1 AND desa_id = " . $this->config->item('desa_id') . ")";
		$query = $this->db->query($sql);
		$data = $query->row_array();
	...
	public function get_data_ayah($id = 0)
	{
		...
		if (empty($data['id']) && !empty($penduduk['ayah_nik'])) {
			$sql = "SELECT u.id
				FROM tweb_penduduk u
				WHERE u.nik = ? AND desa_id = " . $this->config->item('desa_id') . " limit 1";
	...
	{
		$sql = "SELECT * FROM tweb_surat_format WHERE url_surat = ? AND desa_id = " . $this->config->item('desa_id') . "";
