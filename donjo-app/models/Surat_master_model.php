<?php class Surat_master_model extends MY_Model
{
	...
	{
		$sql = " FROM tweb_surat_format u WHERE 1 AND u.desa_id = " . $this->config->item('desa_id') . " ";
		$sql .= $this->search_sql();
		$sql .= $this->filter_sql();
		return $sql;
	}
	...
	{
		$sql = "SELECT * FROM tweb_surat_format WHERE id = ? AND desa_id = " . $this->config->item('desa_id') . "";
	...
	public function impor_surat_desa()
	{
		...
		foreach ($folder_surat_desa as $surat) {
			...
			$hasil = $this->db->where('url_surat', $surat)->where('desa_id', $this->config->item('desa_id'))->get('tweb_surat_format');
			if ($hasil->num_rows() == 0) {
				$data = array();
				$data['jenis'] = 2;
				$data['url_surat'] = $surat;
				$data['nama'] = ucwords(trim(str_replace(array("surat", "-", "_"), ' ', $surat)));
				$sql = $this->db->insert_string('tweb_surat_format', $data + ['desa_id' => $this->config->item('desa_id')]) . " ON DUPLICATE KEY UPDATE jenis = VALUES(jenis), nama = VALUES(nama)";
				$this->db->query($sql);
			}
		}
	}
