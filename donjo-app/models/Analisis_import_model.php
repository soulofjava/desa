...
class Analisis_import_Model extends CI_Model
{
	...
	public function import_excel($file = '', $kode = '00000', $jenis = 2)
	{
		...
		$outp = $this->db->insert('analisis_master', $master + ['desa_id' => $this->config->item('desa_id')]);
		...
		$periode['aktif']	= 1;
		$this->db->insert('analisis_periode', $periode + ['desa_id' => $this->config->item('desa_id')]);
		...
		for ($i = 2; $i <= $baris; $i++) {
			$sql = "SELECT * FROM analisis_kategori_indikator WHERE kategori=? AND id_master=? AND desa_id=" . $this->config->item('desa_id');
			...
			if (!$cek) {
				...
				$this->db->insert('analisis_kategori_indikator', $kategori + ['desa_id' => $this->config->item('desa_id')]);
			}
		}
		...
		for ($i = 2; $i <= $baris; $i++) {
			...
			$sql =
				"SELECT * FROM analisis_kategori_indikator WHERE kategori=? AND id_master=? AND desa_id=" . $this->config->item('desa_id');
			$query = $this->db->query($sql, array($data->val($i, 3, $sheet), $id_master));
			...
			$this->db->insert('analisis_indikator', $indikator + ['desa_id' => $this->config->item('desa_id')]);
		}
		...
		for ($i = 2; $i <= $baris; $i++) {
			...
			$sql =
				"SELECT id FROM analisis_indikator WHERE nomor=? AND id_master=? AND desa_id=" . $this->config->item('desa_id');
		...
		for ($i = 2; $i <= $baris; $i++) {
			...
			$this->db->insert('analisis_klasifikasi', $klasifikasi + ['desa_id' => $this->config->item('desa_id')]);
		}
