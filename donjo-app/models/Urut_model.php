...
class Urut_model extends CI_Model
{
	...
	private function urut_semua($subset = array('1' => '1'))
	{
		...
		for ($i = 0; $i < count($daftar); $i++) {
			$this->db->where($this->kolom_id, $daftar[$i][$this->kolom_id])->where('desa_id', $this->config->item('desa_id'));
			$data['urut'] = $i + 1;
	...
	private function urut_daftar($id, $arah, $daftar, $unsur1)
	{
		...
		// Tukar urutan
		$this->db->where($this->kolom_id, $unsur2[$this->kolom_id])->where('desa_id', $this->config->item('desa_id'))->update($this->tabel, array('urut' => $unsur1['urut']));
		$this->db->where($this->kolom_id, $unsur1[$this->kolom_id])->where('desa_id', $this->config->item('desa_id'))->update($this->tabel, array('urut' => $unsur2['urut']));
