<?php class Bdt_model extends CI_Model
{
	...
	*/
	...
	private function tulis_rtm($baris, &$rtm)
	{
		...
		$query = $this->db->where('nik', $nik)->where('desa_id', $this->config->item('desa_id'))->get('tweb_penduduk');
		if ($query->num_rows() == 0) {
			...
			$rtm = $this->db->where('no_kk', $id_rtm)->where('desa_id', $this->config->item('desa_id'))->get('tweb_rtm')->row()->id;
			if ($rtm) {
				// Update rumah tangga
				if ($rtm_level == 1) {
				...
				$this->db->insert('tweb_rtm', $rtm_data + ['desa_id' => $this->config->item('desa_id')]);
		...
		$this->db->where('nik', $nik)->where('desa_id', $this->config->item('desa_id'))->update('tweb_penduduk', $penduduk);
