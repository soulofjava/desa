...
class Plan_area_model extends MY_Model
{
	...
	{
		$sql = "SELECT COUNT(l.id) AS id FROM area l LEFT JOIN polygon p ON l.ref_polygon = p.id LEFT JOIN polygon m ON p.parrent = m.id WHERE 1 AND l.desa_id = " . $this->config->item('desa_id') . "";
		$sql .= $this->search_sql();
		$sql .= $this->filter_sql();
	...
	public function insert()
	{
		...
		if (!empty($area_file)) {
			if ($tipe_file == "image/jpg" or $tipe_file == "image/jpeg") {
				...
				$data['foto'] = $nama_file;
				$outp = $this->db->insert('area', $data + ['desa_id' => $this->config->item('desa_id')]);
			}
		} else {
			unset($data['foto']);
			$outp = $this->db->insert('area', $data + ['desa_id' => $this->config->item('desa_id')]);
	...
	{
		$sql = "SELECT * FROM polygon WHERE tipe = 2 AND desa_id = " . $this->config->item('desa_id') . "";
	...
	{
		$sql = "SELECT * FROM polygon WHERE tipe = 0 AND desa_id = " . $this->config->item('desa_id') . "";

		if (isset($_SESSION['polygon'])) {
			$sqlx = "SELECT * FROM polygon WHERE id = ? AND desa_id = " . $this->config->item('desa_id') . "";
	...
	{
		$sql = "SELECT * FROM area WHERE id = ? AND desa_id = " . $this->config->item('desa_id') . "";
