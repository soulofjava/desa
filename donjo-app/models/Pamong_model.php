...
class Pamong_model extends CI_Model
{
	...
	public function insert()
	{
		...
		$outp = $this->db->insert('tweb_desa_pamong', $data + ['desa_id' => $this->config->item('desa_id')]);
	...
	{
		$ttd = $this->db->where('pamong_ttd', 1)->where('desa_id', $this->config->item('desa_id'))->get('tweb_desa_pamong')->row_array();
		return $ttd;
	}

	public function get_ub()
	{
		$ub = $this->db->where('pamong_ub', 1)->where('desa_id', $this->config->item('desa_id'))->get('tweb_desa_pamong')->row_array();
		return $ub;
	}
