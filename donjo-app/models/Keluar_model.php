<?php class Keluar_model extends CI_Model
{
	...
	public function list_data($o = 0, $offset = 0, $limit = 0)
	{
		...
		switch ($o) {
			case 1:
				$order_sql = ' ORDER BY u.no_surat * 1';
				break;
			case 2:
				$order_sql = ' ORDER BY u.no_surat * 1 DESC';
				break;
			case 3:
			...
			case 4:
				...
				break;
			case 5:
				$order_sql = ' ORDER BY u.tanggal';
				break;
			case 6:
				$order_sql = ' ORDER BY u.tanggal DESC';
				break;
	...
	public function log_surat($data_log_surat)
	{
		...
		$sql = "SELECT id FROM tweb_surat_format WHERE url_surat = ? AND desa_id = " . $this->config->item('desa_id') . "";
		 ...
		 */ {
			$log_id = $this->db->select('id')->from('log_surat')->where('nama_surat', $nama_surat)->where('desa_id', $this->config->item('desa_id'))->limit(1)->get()->row()->id;
		} else
		// Cetak:
		// Sama dengan aturan Ekspor Dok, hanya URL-NIK-nomor surat-tanggal diambil dari data kolom
		{
			$log_id = $this->db->select('id')->from('log_surat')->where('id_format_surat', $data['id_format_surat'])->where('id_pend', $data['id_pend'])->where('no_surat', $data['no_surat'])->where('DATE_FORMAT(tanggal, "%Y-%m-%d") =', date('Y-m-d'))->where('desa_id', $this->config->item('desa_id'))->limit(1)->get()->row()->id;
		}
		if ($log_id) {
	...
	{
		$sql = "SELECT id, nik, nama FROM tweb_penduduk WHERE status = 1 AND desa_id = " . $this->config->item('desa_id') . "";
		$query = $this->db->query($sql);
		$data = $query->result_array();
	...
	{
		$jml = $this->db->select('count(*) as jml')->where('desa_id', $this->config->item('desa_id'))->get('log_surat')->row()->jml;
		return $jml;
	}

	public function list_tahun_surat()
	{
		$query = $this->db->distinct()->select('YEAR(tanggal) AS tahun')->order_by('YEAR(tanggal)', 'DESC')->where('desa_id', $this->config->item('desa_id'))->get('log_surat')->result_array();
		return $query;
	}

	public function list_jenis_surat()
	{
		$query = $this->db->distinct()->select('k.nama as nama_surat')->from('log_surat u')->join('tweb_surat_format k', 'u.id_format_surat = k.id', 'left')->where('u.desa_id', $this->config->item('desa_id'))->order_by('nama_surat')->get()->result_array();
		return $query;
	}
}
