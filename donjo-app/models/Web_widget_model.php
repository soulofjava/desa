<?php class Web_widget_model extends MY_Model
{
	...
	{
		$data = $this->db->where('id', $id)->where('desa_id', $this->config->item('desa_id'))->get('widget')->row_array();
	...
	{
		$sql = " FROM widget WHERE 1 AND desa_id=" . $this->config->item('desa_id') . " ";
		$sql .= $this->search_sql();
		$sql .= $this->filter_sql();
	...
	public function insert()
	{
		...
		$outp = $this->db->insert('widget', $data + ['desa_id' => $this->config->item('desa_id')]);
	...
	public function update_setting($widget, $setting)
	{
		...
		$outp = $this->db->where('isi', $widget . '.php')->where('desa_id', $this->config->item('desa_id'))->update('widget', $data);
		if (!$outp) $_SESSION['success'] = -1;
	}
