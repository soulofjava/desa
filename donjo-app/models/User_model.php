...
class User_model extends CI_Model
{
	...
	public function siteman()
	{
		...
		$sql = "SELECT id, password, id_grup, session FROM user WHERE username = ? AND desa_id = " . $this->config->item('desa_id');
		...
		// Login sukses: ubah pass di db ke bcrypt jika masih md5 dan set session
			...
			if ($pwMasihMD5) {
				...
				$sql =
					"UPDATE user SET password = ? WHERE id = ? AND desa_id = " . $this->config->item('desa_id');
	...
	{
		$sql =
			"UPDATE user SET last_login = NOW() WHERE id = ? AND desa_id = " . $this->config->item('desa_id');
	...
	{
		$sql =
			"SELECT id_grup FROM user WHERE session = ? AND desa_id = " . $this->config->item('desa_id');
	...
	public function login()
	{
		...
		$sql =
			"SELECT id, password, id_grup, session FROM user WHERE id_grup = 1AND desa_id = " . $this->config->item('desa_id') . " LIMIT 1";
	...
	public function insert()
	{
		...
		$sql =
			"SELECT username FROM user WHERE username = ? AND desa_id = " . $this->config->item('desa_id');
		...
		if (!$this->db->insert('user', $data + ['desa_id' => $this->config->item('desa_id')])) {
			$this->session->success = -1;
	...
	public function update($idUser)
	{
		...
		if (!$this->db->where('id', $idUser)->where('desa_id', $this->config->item('desa_id'))->update('user', $data)) {
			$this->session->success = -1;
			$this->session->error_msg = ' -> Gagal memperbarui data di database';
		}
	}
	...
	{
		$sql =
			"SELECT * FROM user WHERE id = ? AND desa_id = " . $this->config->item('desa_id');
