<?php class Teks_berjalan_model extends MY_Model
{
	...
	public function get_teks($id = '')
	{
		...
		$data = $this->db->where('t.id', $id)->where('t.desa_id', $this->config->item('desa_id'))->get()->row_array();
	...
	private function sql()
	{
		$this->db
			...
			->where('t.desa_id', $this->config->item('desa_id'))
			->order_by('urut');
	}
	...
	public function insert()
	{
		...
		$outp = $this->db->insert('teks_berjalan', $data + ['desa_id' => $this->config->item('desa_id')]);
