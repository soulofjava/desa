<?php class Web_gallery_model extends MY_Model
{
	...
	{
		$sql = " FROM gambar_gallery WHERE tipe = 0 AND desa_id=" . $this->config->item('desa_id') . " ";
		$sql .= $this->search_sql();
		$sql .= $this->filter_sql();
		return $sql;
	}
	...
	public function insert()
	{
		...
		$outp = $this->db->insert('gambar_gallery', $data + ['desa_id' => $this->config->item('desa_id')]);
	...
	{
		$sql = "SELECT * FROM gambar_gallery WHERE id = ? AND desa_id=" . $this->config->item('desa_id') . " ";
	...
	{
		$sql = " FROM gambar_gallery WHERE parrent = ? AND tipe = 2 desa_id=" . $this->config->item('desa_id') . " ";
		$sql .= $this->search_sql();
		$sql .= $this->filter_sql();
		return $sql;
	}
