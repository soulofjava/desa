...
*/
	...
	protected function tulis_tweb_keluarga(&$isi_baris)
	{
		...
		if (!empty($res)) {
			...
			$data['alamat'] = $isi_baris['alamat'];
			$hasil = $this->db->insert('tweb_keluarga', $data + ['desa_id' => $this->config->item('desa_id')]);
	...
	protected function tulis_tweb_penduduk($isi_baris)
	{
		...
		if ($isi_baris['nik'] != 0) {
			...
			if (!empty($res)) {
				...
				if (!$this->db->insert('tweb_penduduk', $data + ['desa_id' => $this->config->item('desa_id')])) $this->error_tulis_penduduk = $this->db->error();;
		...
		foreach ($reader->getSheetIterator() as $sheet) {
			...
			foreach ($sheet->getRowIterator() as $row) {
				...
				if ($pdd) {
					...
					$outp = &$this->db->insert('tweb_penduduk', $penduduk + ['desa_id' => $this->config->item('desa_id')]);
