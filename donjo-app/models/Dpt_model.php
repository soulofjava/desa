<?php class Dpt_model extends Penduduk_model
{
	...
	protected function umur_max_sql()
	{
		if (isset($_SESSION['umur_max'])) {
			...
			$umur_max_sql = " AND (SELECT DATE_FORMAT(FROM_DAYS(TO_DAYS(STR_TO_DATE('$tanggal_pemilihan','%d-%m-%Y'))-TO_DAYS(`tanggallahir`)), '%Y')+0 FROM tweb_penduduk WHERE id = u.id AND desa_id = " . $this->config->item('desa_id') . ") <= $kf ";
			return $umur_max_sql;
		}
	}
	...
	protected function umur_min_sql()
	{
		if (isset($_SESSION['umur_min'])) {
			...
			$umur_min_sql = " AND (SELECT DATE_FORMAT(FROM_DAYS(TO_DAYS(STR_TO_DATE('$tanggal_pemilihan','%d-%m-%Y'))-TO_DAYS(`tanggallahir`)), '%Y')+0 FROM tweb_penduduk WHERE id = u.id AND desa_id = " . $this->config->item('desa_id') . ") >= $kf ";
			return $umur_min_sql;
		}
	}
	...
	protected function umur_sql()
	{
		if (isset($_SESSION['umurx'])) {
			$kf = $_SESSION['umurx'];
			if ($kf != BELUM_MENGISI)
				$umur_sql = " AND (SELECT DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(`tanggallahir`)), '%Y')+0 FROM tweb_penduduk WHERE id = u.id AND desa_id = " . $this->config->item('desa_id') . ") >= (SELECT dari FROM tweb_penduduk_umur WHERE id=$kf AND desa_id = " . $this->config->item('desa_id') . ") AND (SELECT DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(`tanggallahir`)), '%Y')+0 FROM tweb_penduduk WHERE id = u.id AND desa_id = " . $this->config->item('desa_id') . ") <= (SELECT sampai FROM tweb_penduduk_umur WHERE id=$kf AND desa_id = " . $this->config->item('desa_id') . ") ";
			else $umur_sql = '';
			return $umur_sql;
		}
	}
	...
	*/
	private function syarat_dpt_sql()
	{
		...
		$sql .= " AND (((SELECT DATE_FORMAT(FROM_DAYS(TO_DAYS(STR_TO_DATE('$tanggal_pemilihan','%d-%m-%Y'))-TO_DAYS(`tanggallahir`)), '%Y')+0 FROM tweb_penduduk WHERE id = u.id AND desa_id = " . $this->config->item('desa_id') . ") >= 17) OR u.status_kawin IN (2,3,4))";
	...
	public function list_data($o = 0, $offset = 0, $limit = 0)
	{
		...
		$select_sql = "SELECT DISTINCT u.id,u.nik,u.tanggallahir,u.tempatlahir,u.status,u.status_dasar,u.id_kk,u.nama,u.nama_ayah,u.nama_ibu,a.dusun,a.rw,a.rt,d.alamat,d.no_kk AS no_kk,
			(SELECT DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(`tanggallahir`)), '%Y')+0 FROM tweb_penduduk WHERE id = u.id AND desa_id = " . $this->config->item('desa_id') . ") AS umur,(SELECT DATE_FORMAT(FROM_DAYS(TO_DAYS(STR_TO_DATE('$tanggal_pemilihan','%d-%m-%Y'))-TO_DAYS(`tanggallahir`)), '%Y')+0 FROM tweb_penduduk WHERE id = u.id AND desa_id = " . $this->config->item('desa_id') . ") AS umur_pada_pemilihan, x.nama AS sex,sd.nama AS pendidikan_sedang,n.nama AS pendidikan,p.nama AS pekerjaan,k.nama AS kawin,g.nama AS agama,m.nama AS gol_darah,hub.nama AS hubungan
			";
		//Main Query
		$list_data_sql = $this->list_data_sql($log);
		...
		switch ($o) {
			...
			case 4:
				...
				break;
			case 5:
				$order_sql = ' ORDER BY d.no_kk';
				break;
			case 6:
				$order_sql = ' ORDER BY d.no_kk DESC';
				break;
			case 7:
			...
			case 8:
				...
				// Untuk Log Penduduk
			case 9:
				$order_sql = ' ORDER BY log.tgl_peristiwa';
				break;
			case 10:
				$order_sql = ' ORDER BY log.tgl_peristiwa DESC';
				break;
			default:
				$order_sql = '';
		}
