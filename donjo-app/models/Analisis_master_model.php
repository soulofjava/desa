...
class Analisis_master_model extends MY_Model
{
	...
	{
		$sql =
			"SELECT COUNT(id) AS id FROM analisis_master u WHERE 1 AND desa_id=" . $this->config->item('desa_id');
		$sql .= $this->search_sql();
		$sql .= $this->filter_sql();
		$sql .= $this->state_sql();
	...
	public function list_data($o = 0, $offset = 0, $limit = 500)
	{
		switch ($o) {
			case 1:
				$order_sql = ' ORDER BY u.lock';
				break;
			case 2:
				$order_sql = ' ORDER BY u.lock DESC';
				break;
			case 3:
			...
			case 4:
				...
				break;
			case 5:
				$order_sql = ' ORDER BY s.subjek';
				break;
			case 6:
				$order_sql = ' ORDER BY s.subjek DESC';
				break;
			default:
		...
		$sql =
			"SELECT u.*,s.subjek FROM analisis_master u LEFT JOIN analisis_ref_subjek s ON u.subjek_tipe = s.id WHERE 1 AND u.desa_id=" . $this->config->item('desa_id');
	...
	public function insert()
	{
		...
		$outp = $this->db->insert('analisis_master', $data + ['desa_id' => $this->config->item('desa_id')]);
		status_sukses($outp);
	}
	...
	{
		$sql =
			"SELECT * FROM analisis_master WHERE subjek_tipe = 1 AND desa_id=" . $this->config->item('desa_id');
		$query = $this->db->query($sql);
		return $query->result_array();
	}
}
