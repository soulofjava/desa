<?php class Analisis_periode_model extends MY_Model
{
	...
	{
		$sql =
			"SELECT COUNT(id) AS id FROM analisis_periode u WHERE 1 AND desa_id=" . $this->config->item('desa_id');
		$sql .= $this->search_sql();
		$sql .= $this->master_sql();
		$sql .= $this->state_sql();
	...
	public function list_data($o = 0, $offset = 0, $limit = 500)
	{
		switch ($o) {
			...
			case 4:
				...
				break;
			case 5:
				$order_sql = ' ORDER BY g.id';
				break;
			case 6:
				$order_sql = ' ORDER BY g.id DESC';
				break;
			default:
				$order_sql = ' ORDER BY u.id';
		}
		...
		$sql =
			"SELECT u.*,s.nama AS status FROM analisis_periode u LEFT JOIN analisis_ref_state s ON u.id_state = s.id WHERE 1 AND u.desa_id=" . $this->config->item('desa_id');
	...
	public function insert()
	{
		...
		if ($dp == 1) {
			$sqld =
				"SELECT id FROM analisis_periode WHERE id_master = ? AND desa_id = " . $this->config->item('desa_id') . " ORDER BY id DESC LIMIT 1 ";
		...
		$outp = $this->db->insert('analisis_periode', $data + ['desa_id' => $this->config->item('desa_id')]);
		...
		if ($dp == 1) {
			...
			$sql 	=
				"SELECT id_subjek,id_indikator,id_parameter FROM analisis_respon WHERE id_periode = ? AND desa_id=" . $this->config->item('desa_id');
	...
	{
		$sql =
			"SELECT * FROM analisis_periode WHERE id = ? AND desa_id=" . $this->config->item('desa_id');
