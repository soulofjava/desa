...
class First_menu_m extends MY_Model
{
	...
	{
		$data	= $this->db->select('*')->where(array('parrent' => $menu_id, 'enabled' => 1, 'tipe' => 3))
			->where('desa_id', $this->config->item('desa_id'))->order_by('urut')->get('menu')->result_array();
	...
	{
		$sql = "SELECT m.* FROM menu m WHERE m.parrent = 1 AND m.enabled = 1 AND m.tipe = 1 AND m.desa_id = " . $this->config->item('desa_id') . " order by urut asc";
