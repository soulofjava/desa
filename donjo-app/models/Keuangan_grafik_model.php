...
class Keuangan_grafik_model extends CI_model
{
	...
	public function rp_apbd_widget($thn, $opt = false)
	{
		...
		$data['jenis_pelaksanaan'] = $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_master')->result_array();

		$this->db->select('LEFT(Kd_Rincian, 2) AS jenis_pelaksanaan, SUM(AnggaranStlhPAK) AS pagu');
		$this->db->where('Tahun', $thn);
		$this->db->group_by('jenis_pelaksanaan');
		$data['anggaran'] = $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_ta_rab_rinci')->result_array();

		$this->db->select('LEFT(Kd_Rincian, 2) AS jenis_pelaksanaan, SUM(Nilai) AS realisasi');
		$this->db->group_by('jenis_pelaksanaan');
		$this->db->where('Tahun', $thn);
		$data['realisasi_pendapatan'] = $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_ta_tbp_rinci')->result_array();
		...
		$data['realisasi_belanja'] = $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_ta_spp_rinci')->result_array();

		$this->db->select('LEFT(Kd_Rincian, 2) AS jenis_pelaksanaan, SUM(Nilai) AS realisasi');
		$this->db->where('Tahun', $thn);
		$this->db->group_by('jenis_pelaksanaan');
		$this->db->like('Kd_Rincian', '5.', 'after');
		$data['realisasi_belanja_spj'] = $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_ta_spj_rinci')->result_array();

		$this->db->select('LEFT(Kd_Rincian, 2) AS jenis_pelaksanaan, SUM(Nilai) AS realisasi');
		$this->db->group_by('jenis_pelaksanaan');
		$this->db->where('Tahun', $thn);
		$data['realisasi_bunga'] = $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_ta_mutasi')->result_array();

		$this->db->select('LEFT(Kd_Rincian, 2) AS jenis_pelaksanaan, SUM(Kredit) AS realisasi');
		$this->db->group_by('jenis_pelaksanaan');
		$this->db->where('Tahun', $thn);
		$data['realisasi_biaya'] = $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_ta_jurnal_umum_rinci')->result_array();
	...
	public function r_pd_widget($thn, $opt = false)
	{
		...
		$data['jenis_pendapatan'] = $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_master')->result_array();
	...
	public function r_bd_widget($thn, $opt = false)
	{
		...
		$data['jenis_belanja'] = $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_master')->result_array();
  ...
  */
	...
	public function lap_rp_apbd($thn, $smt1 = false)
	{
		...
		$this->db->where('tahun_anggaran', $thn);
		$data['pendapatan'] = $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_master')->result_array();
		...
		$this->db->where('tahun_anggaran', $thn);
		$data['belanja'] = $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_master')->result_array();
		...
		$this->db->join('keuangan_ta_bidang', 'keuangan_ta_bidang.id_keuangan_master = keuangan_master.id', 'left');
		$this->db->where('tahun_anggaran', $thn);
		$data['belanja_bidang'] = $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_master')->result_array();
		...
		$this->db->where('tahun_anggaran', $thn);
		$data['pembiayaan'] = $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_master')->result_array();
		...
		$this->db->where("Akun = '6.'");
		$this->db->where('tahun_anggaran', $thn);
		$data['pembiayaan_keluar'] = $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_master')->result_array();
	...
	private function pagu_akun($akun, $thn)
	{
		...
		$this->db->where('Tahun', $thn);
		$this->db->group_by('Akun');
		return $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_ta_rab_rinci')->result_array();
	}
	...
	private function real_akun_pendapatan($akun, $thn, $smt1 = false)
	{
		...
		$this->db->group_by('Akun');
		return $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_ta_tbp_rinci')->result_array();
	}
	...
	private function real_akun_pendapatan_bunga($akun, $thn, $smt1 = false)
	{
		...
		$this->db->group_by('Akun');
		return $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_ta_mutasi')->result_array();
	}
	...
	private function real_akun_belanja($akun, $thn, $smt1 = false)
	{
		...
		$this->db->group_by('Akun');
		return $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_ta_spp_rinci')->result_array();
	}
	...
	private function real_akun_belanja_spj($akun, $thn, $smt1 = false)
	{
		...
		$this->db->group_by('Akun');
		return $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_ta_spj_rinci')->result_array();
	}
	...
	private function get_subval_pendapatan($id_keuangan_master, $akun, $thn, $smt1 = false)
	{
		...
		$data = $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_ref_rek2')->result_array();
		foreach ($data as $i => $d) {
	...
	private function get_subval_belanja_bidang($id_keuangan_master, $akun, $thn, $smt1 = false)
	{
		...
		$data = $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_ta_kegiatan')->result_array();
		foreach ($data as $i => $d) {
	...
	private function real_subval_pembiayaan($kelompok, $thn, $smt1 = false)
	{
		...
		$this->db->group_by('Kelompok');
		return $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_ta_jurnal_umum_rinci')->result_array();
	}
	...
	private function sub_pendapatan2($id_keuangan_master, $kelompok, $thn, $smt1 = false)
	{
		...
		$this->db->where('id_keuangan_master', $id_keuangan_master);
		$data = $this->db->where('desa_id', $this->config->item('desa_id'))->get('keuangan_ref_rek3')->result_array();
		foreach ($data as $i => $d) {
