...
class Program_bantuan_model extends MY_Model
{
	...
	public function list_program($sasaran = 0)
	{
		if ($sasaran > 0) {
			$strSQL = "SELECT p.id, p.nama, p.sasaran, p.ndesc, p.sdate, p.edate, p.userid, p.status
				FROM program p WHERE p.sasaran=" . $sasaran . " AND p.desa_id = " . $this->config->item('desa_id');
		} else {
			$strSQL = "SELECT p.id, p.nama, p.sasaran, p.ndesc, p.sdate, p.edate, p.userid, p.status, CONCAT('50',p.id) as lap
           FROM program p 
           WHERE 1 AND p.desa_id = " . $this->config->item('desa_id');
		}
	...
	public function list_program_keluarga($kk_id)
	{
		...
		$strSQL = "
			...
			WHERE p.sasaran = $sasaran and p.desa_id = " . $this->config->item('desa_id');
		$query = $this->db->query($strSQL);
		$data = $query->result_array();
	...
	*/
		...
		switch ($sasaran) {
			case 1:
				...
				$strSQL = "SELECT " . $select_sql . " FROM program_peserta p
					...
					. " AND p.desa_id = " . $this->config->item('desa_id');
				break;
	...
	{
		$sql = ' FROM program p WHERE 1 and p.desa_id =' . $this->config->item('desa_id');
	...
	private function get_pilihan_penduduk($filter)
	{
		...
		$strSQL = "SELECT p.nik, p.nama, w.rt, w.rw, w.dusun
			...
			WHERE 1 AND p.desa_id = $this->config->item('desa_id') ORDER BY nama";
		$query = $this->db->query($strSQL);
		$data = "";
		$data = $query->result_array();
		if ($query->num_rows() > 0) {
	...
	public function get_program($p, $slug)
	{
		if ($slug === false) {
			...
			$strSQL .= " WHERE p.desa_id =" . $this->config->item('desa_id');
	...
	function get_data_program($id)
	{
		...
		return $this->db->select("*")->where("id", $program_id)->where("desa_id", $this->config->item('desa_id'))->get("program")->row_array();
	}
	...
	public function get_peserta_program($cat, $id)
	{
		...
		$strSQL = "SELECT p.id AS id, o.peserta AS nik, o.id AS peserta_id,  p.nama AS nama, p.sdate, p.edate, p.ndesc, p.status
			...
			WHERE ((o.peserta='" . $id . "') AND (p.sasaran='" . $cat . "') AND (o.desa_id='" . $this->config->item('desa_id') . "'))";
		$query = $this->db->query($strSQL);
		if ($query->num_rows() > 0) {
		...
		switch ($cat) {
			case 1:
				...
				$strSQL = "SELECT o.nama, o.foto, o.nik, w.rt, w.rw, w.dusun
					...
					LEFT JOIN tweb_wil_clusterdesa w ON w.id = o.id_cluster
					WHERE o.nik='" . $id . "' AND o.desa_id='" . $this->config->item('desa_id') . "'";
				$query = $this->db->query($strSQL);
				if ($query->num_rows() > 0) {
					$row = $query->row_array();
					$data_profil = array(
			...
			case 2:
				...
				$strSQL = "SELECT o.nik_kepala, o.no_kk, p.nama, w.rt, w.rw, w.dusun FROM tweb_keluarga o
					...
					LEFT JOIN tweb_wil_clusterdesa w ON w.id = p.id_cluster WHERE o.no_kk = '" . $id . "' AND (o.desa_id='" . $this->config->item('desa_id') . "')) ";
				$query = $this->db->query($strSQL);
				if ($query->num_rows() > 0) {
					$row = $query->row_array();
					$data_profil = array(
			...
			case 3:
				...
				$strSQL = "SELECT r.id, r.no_kk, o.nama, o.nik, w.rt, w.rw, w.dusun  FROM tweb_rtm r
					LEFT JOIN tweb_penduduk o ON o.id = r.nik_kepala
					LEFT JOIN tweb_wil_clusterdesa w ON w.id = o.id_cluster
					WHERE r.no_kk=$id AND (o.desa_id='" . $this->config->item('desa_id') . "'))";
				$query = $this->db->query($strSQL);
				if ($query->num_rows() > 0) {
					$row = $query->row_array();
					$data_profil = array(
			...
			case 4:
				...
				$strSQL = "SELECT k.id as id, k.nama as nama, p.nama as ketua, p.nik as nik, w.rt, w.rw, w.dusun FROM kelompok k
				 ...
				 LEFT JOIN tweb_wil_clusterdesa w ON w.id = p.id_cluster
				 WHERE k.id='" . $id . "' AND (o.desa_id='" . $this->config->item('desa_id') . "'))";
				$query = $this->db->query($strSQL);
				if ($query->num_rows() > 0) {
					$row = $query->row_array();
					$data_profil = array(
	...
	public function set_program()
	{
		...
		return $this->db->insert('program', $data + ['desa_id' => $this->config->item('desa_id')]);
	}
	...
	public function add_peserta($program_id)
	{
		...
		$outp = $this->db->insert('program_peserta', $data) + ['desa_id' => $this->config->item('desa_id')];
	...
	*/
	...
	{
		$jml_peserta = $this->db->select('count(v.program_id) as jml')->from('program p')->join('program_peserta v', 'p.id = v.program_id', 'left')->where('p.id', $id)->where('p.desa_id', $this->config->item('desa_id'))->get()->row()->jml;
