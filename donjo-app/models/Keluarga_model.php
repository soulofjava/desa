<?php class Keluarga_model extends CI_Model
{
	...
	*/
	...
	private function list_data_sql()
	{
		...
		$sql .= " WHERE 1 AND u.desa_id = " . $this->config->item('desa_id') . "";
		$sql .=	$this->search_sql();
	...
	public function list_data($o = 0, $offset = 0, $limit = 0)
	{
		...
		switch ($o) {
			case 1:
				$order_sql = ' ORDER BY u.no_kk';
				break;
			case 2:
				$order_sql = ' ORDER BY u.no_kk DESC';
				break;
			case 3:
				$order_sql = ' ORDER BY kepala_kk';
				break;
			case 4:
				$order_sql = ' ORDER BY kepala_kk DESC';
				break;
			case 5:
				$order_sql = ' ORDER BY u.tgl_daftar';
				break;
			case 6:
				$order_sql = ' ORDER BY u.tgl_daftar DESC';
				break;
			default:
				$order_sql = ' ORDER BY u.no_kk DESC';
		}
		...
		$sql = "SELECT u.*, t.nama AS kepala_kk, t.nik, t.tag_id_card, t.sex, t.status_dasar, t.foto, t.id as id_pend,
			(SELECT COUNT(id) FROM tweb_penduduk WHERE id_kk = u.id AND status_dasar = 1 AND desa_id = " . $this->config->item('desa_id') . ") AS jumlah_anggota,
			c.dusun, c.rw, c.rt ";
	...
	public function insert()
	{
		...
		$pend = $this->db->select('alamat_sekarang, id_cluster')->where('id', $data['nik_kepala'])->where('desa_id', $this->config->item('desa_id'))->get('tweb_penduduk')->row_array();
		// Gunakan alamat penduduk sebagai alamat keluarga
		$data['alamat'] = $pend['alamat_sekarang'];
		$data['id_cluster'] = $pend['id_cluster'];

		$outp = $this->db->insert('tweb_keluarga', $data + ['desa_id' => $this->config->item('desa_id')]);
		...
		$outp = $this->db->insert('log_perubahan_penduduk', $log + ['desa_id' => $this->config->item('desa_id')]);
	...
	private function validasi_data_keluarga(&$data)
	{
		...
		if (isset($data['no_kk'])) {
			...
			if ($this->db->select('no_kk')->from('tweb_keluarga')->where(array('no_kk' => $data['no_kk']))->where('desa_id', $this->config->item('desa_id'))->limit(1)->get()->row()->no_kk)
	...
	public function insert_new()
	{
		...
		$outp = $this->db->insert('tweb_keluarga', $data2 + ['desa_id' => $this->config->item('desa_id')]);
		$kk_id = $this->db->insert_id();
	...
	*/
	public function log_keluarga($id, $kk, $id_peristiwa)
	{
		...
		$outp = $this->db->insert('log_keluarga', $log_keluarga + ['desa_id' => $this->config->item('desa_id')]);
	}
	...
	public function rem_anggota($kk = 0, $id = 0)
	{
		...
		$temp['no_kk_sebelumnya'] = $this->db->select('no_kk')->where('id', $kk)->where('desa_id', $this->config->item('desa_id'))->get('tweb_keluarga')->row()->no_kk;
		$temp['id_kk'] = 0;
		$temp['kk_level'] = 0;
		$temp['updated_at'] = date('Y-m-d H:i:s');
		$temp['updated_by'] = $this->session->user;
	...
	public function get_keluarga($id = 0)
	{
		$sql = "SELECT k.*, b.dusun as dusun, b.rw as rw
			...
			WHERE k.id = ? AND k.desa_id = " . $this->config->item('desa_id') . "";
		$query = $this->db->query($sql, $id);
	...
	{
		$sql = "SELECT * FROM tweb_penduduk WHERE id = ? AND desa_id = " . $this->config->item('desa_id') . "";
		$query = $this->db->query($sql, $id);
		$data = $query->row_array();
	...
	public function list_anggota($id = 0, $options = array('dengan_kk' => true))
	{
		$sql = "SELECT u.*, u.sex as sex_id, u.status_kawin as status_kawin_id,
			(SELECT DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(`tanggallahir`)), '%Y')+0 FROM tweb_penduduk WHERE id = u.id AND desa_id = " . $this->config->item('desa_id') . ") AS umur,
	...
	public function get_kepala_a($id)
	{
		$sql = "SELECT u.*, c.*, k.no_kk, k.alamat
			...
			LEFT JOIN tweb_wil_clusterdesa c ON u.id_cluster = c.id WHERE u.id = (SELECT nik_kepala FROM tweb_keluarga WHERE id = ? AND desa_id = " . $this->config->item('desa_id') . ")
			WHERE u.desa_id = " . $this->config->item('desa_id') . "
			";
	...
	{
		$sql = "SELECT *,nama as hubungan FROM tweb_penduduk_hubungan WHERE 1 AND desa_id = " . $this->config->item('desa_id') . "";
		$query = $this->db->query($sql);
	...
	{
		$this->db->where('id', $id_kk)->update('tweb_keluarga', array('id_cluster' => $id_cluster));
	...
	private function pindah_anggota_keluarga($id_kk, $id_cluster)
	{
		...
		if (!empty($id_cluster)) {
			...
			$sql = "SELECT id FROM tweb_penduduk WHERE id_kk = $id_kk AND desa_id = " . $this->config->item('desa_id') . "";
			$query = $this->db->query($sql);
	...
	public function get_judul_statistik($tipe = 0, $nomor = 1, $sex = 0)
	{
		if ($nomor == JUMLAH)
			...
			switch ($tipe) {
				case 'kelas_sosial':
					$sql = "SELECT * FROM tweb_keluarga_sejahtera WHERE id = ? AND desa_id = " . $this->config->item('desa_id') . "";
					break;
				case 'bantuan_keluarga':
					$sql = "SELECT * FROM program WHERE id = ? AND desa_id = " . $this->config->item('desa_id') . "";
					break;
			}
