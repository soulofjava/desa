...
class Data_persil_model extends MY_Model
{
	...
	public function simpan_persil($post)
	{
		...
		if ($id_persil) {
			...
			$this->db->insert('persil', $data + ['desa_id' => $this->config->item('desa_id')]);
	...
	{
		$strSQL = "SELECT `id`,`rt`,`rw`,`dusun` FROM `tweb_wil_clusterdesa` WHERE (`rt`>0) AND desa_id = " . $this->config->item('desa_id') . " ORDER BY `dusun`";
	...
	private function mutasi_awal($data, $id_persil)
	{
		...
		$this->db->insert('mutasi_cdesa', $mutasi + ['desa_id' => $this->config->item('desa_id')]);
	}
}
