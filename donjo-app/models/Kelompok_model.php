...
class Kelompok_model extends MY_Model
{
	...
	public function list_data($o = 0, $offset = 0, $limit = 0)
	{
		switch ($o) {
			...
			case 2:
				...
				break;
			case 3:
				$order_sql = ' ORDER BY c.nama';
				break;
			case 4:
				$order_sql = ' ORDER BY c.nama DESC';
				break;
			case 5:
				$order_sql = ' ORDER BY master';
				break;
			case 6:
				$order_sql = ' ORDER BY master DESC';
				break;
			default:
				$order_sql = ' ORDER BY u.nama';
		}
		...
		$select_sql = "SELECT u.*, s.kelompok AS master, c.nama AS ketua, (SELECT COUNT(id) FROM kelompok_anggota WHERE id_kelompok = u.id AND desa_id = " . $this->config->item('desa_id') . ") AS jml_anggota ";
	...
	public function insert()
	{
		...
		$outpa = $this->db->insert('kelompok', $data + ['desa_id' => $this->config->item('desa_id')]);
		...
		$outpb = $this->db
			...
			->set('desa_id', $this->config->item('desa_id'))
	...
	public function get_ketua_kelompok($id)
	{
		...
		$sql = "SELECT u.id, u.nik, u.nama, k.id as id_kelompok, k.nama as nama_kelompok, u.tempatlahir, u.tanggallahir, s.nama as sex,
			...
			WHERE k.id = $id AND desa_id = " . $this->config->item('desa_id') . " LIMIT 1";
		$query = $this->db->query($sql);
	...
	{
		$sql = "SELECT * FROM kelompok_anggota WHERE id_kelompok = ? AND id_penduduk = ? AND desa_id = " . $this->config->item('desa_id') . "";
	...
	{
		$sql = "SELECT * FROM kelompok_master WHERE desa_id = " . $this->config->item('desa_id') . "";
		$query = $this->db->query($sql);
	...
	public function list_penduduk($ex_kelompok = '')
	{
		if ($ex_kelompok) {
			...
			if ($anggota) $this->db->where("p.id not in ($anggota)")->where('p.desa_id', $this->config->item('desa_id'));
		}
	...
	public function list_anggota($id_kelompok = 0, $sub = '')
	{
		...
		if ($sub == 'anggota') $this->db->where('jabatan', 90)->where('desa_id', $this->config->item('desa_id')); // Hanya anggota saja, tidak termasuk pengurus
		$data = $this->db
			->select('ka.*, tp.nik, tp.nama, tp.tempatlahir, tp.tanggallahir, tpx.nama AS sex')
			->select("(SELECT DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(tanggallahir)), '%Y')+0 FROM tweb_penduduk WHERE id = tp.id AND desa_id = " . $this->config->item('desa_id') . ") AS umur")
			...
			->where('ka.desa_id', $this->config->item('desa_id'))
